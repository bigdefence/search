...
async def fetch_all_search_results(query):
    ...
    google_video_task = google_search(query, GOOGLE_API_KEY, CSE_ID, num_results=num_results_google, search_type="video")
    ...
    google_text, google_images, google_videos, naver_text, naver_images, naver_videos = await asyncio.gather(
        google_text_task, google_image_task, google_video_task,
        naver_text_task, naver_image_task, naver_video_task
    )
...
def extract_keywords_openai(query):
    prompt = f"""
    Extract 2-4 essential keywords or short phrases from the following query. Focus only on:
    1. Main topics or core concepts
    2. Technical terms or names of technologies
    3. Specific names of places, products, or services

    Ignore common words, articles, and auxiliary verbs. Keep original language (Korean or English) for extracted keywords.
    
    Query: "{query}"
    Output only the keywords, separated by commas, without any additional text.
    """
    try:
        response = openai.chat.completions.create(
            model="gpt-4o-mini",
